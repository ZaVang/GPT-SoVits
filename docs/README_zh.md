...
- **批量推理 API**：现在可以上传一个Excel文件进行批量推理。Excel文件应包含两列，格式如下：

  - 第一列：文本内容text，第二列：文件名filename，第一行为表头
  - 该功能会将text转换成音频并保存在对应的 `filenames.wav` 文件中。
- **自动选择参考音频功能**：只需输入角色的名字，系统会自动选择模型参考音频和语种。
